(ns user
  ...
  (:require
    ...
    [kit.translation-reader.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (kit.translation-reader.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (kit.translation-reader.config/system-config {:profile :test})
                                  (ig/prep)))))
